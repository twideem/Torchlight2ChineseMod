...
請下載 bin 資料夾裡的 CHINESE.MOD，然後放在 [使用者文件]\My Games\Runic Games\Torchlight 2\mods 裡面，並以掛載模組的方式，掛載本中文化進行遊戲。

如果你想修改本中文化，請下載 src 資料夾裡面的檔案，以遊戲隨附的編輯器 GUTS 製作成模組。

## 注意

行尾空白與否，視為不同項目，所以用 Notepad++ 編輯前不要啟用存檔刪除結尾空白的功能！
